...
var eventbus = {
    ...
    triggerHandler: function( scope, event, args ) {
        var handler = select( this._handlers, scope ).select( event )[0],
            args = [ scope, event ].concat( args );
        if ( handler && handler.apply ) { handler.apply( null, args ); }
    ...
    triggerListeners: function( scope, event, args ) {
        ...
        listeners.map( function( listener ) {
            if ( listener.apply ) { listener.apply( null, args ); }
        });
     ...
     */
    trigger: function( scope, event, args ) {
