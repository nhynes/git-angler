    ...
    return function( req, res ) {
        ...
        if ( parsedUrl.pathname === '/' ) {
            if ( hookHandlers[ hook ] && repoPath ) {
                repoPathParser = hook.indexOf('receive') ? getLocalRepo : getRemoteRepo;
                repoStr = repoPathParser( repoPath );
                if ( hook.indexOf('receive') !== -1 ) {
                    repoStr = getLocalRepo( repoPath );
                } else {
                    repoStr = getRemoteRepo( repoPath );
                }
